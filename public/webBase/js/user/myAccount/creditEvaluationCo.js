...
//				console.log(res)
				...
				if(res.data.records.length>0) {
						var srRadar1 = [], //存储查询6大项
							...
							badrecord_score = 14, //不良信息
							bankoverdue_score = 14, //银行逾期
							broken_score = res.data.records[0].broken_score, //失信人
							execute_score = 14, //被执行人 
							involvedappeal = 14, //涉诉
							onlineloan_score = 14; //网贷黑名单
						//console.log(new Date().getMonth()+1); //查询记录的最近的当前月份
...
//				console.log(res)
	...
	//企业   信用评估失信人
	function companyDishonest() {
		var url = api + 'webAccount/CreditManagementApi/promiseCompany',
			data_form = {
				'user_id': user_id
			}
		$.ajax({
	...
	//信用评估被执行人 企业
	function srExecutor() {
		var url = api + 'webAccount/CreditManagementApi/executorCompany',
			data_form = {
				'user_id': user_id
			}
		$.ajax({
